...
class DrawingReaderSubAgent:
    ...
    def run(self, file_path, method="auto", lang="en"):
        ...
        text = self._clean_drawing_text(text)
    ...
    def _clean_drawing_text(self, text):
        import re

        # Split into lines
        lines = text.split("\n")

        # Set to track seen content and avoid duplicates
        seen_content = set()
        kept_lines = []

        # Regex patterns
        # 1. Number only (e.g., "12", "14A")
        # 2. Reference (e.g., "FIG. 1", "Figure 2")
        # 3. Number + Text (e.g., "12 Housing", "14A Valve")
        # We want to filter out lines that are JUST text without numbers/references
        # BUT: Sometimes OCR splits "(501)" and "Computers" into two lines.
        # So we should also keep short text lines that might be labels.
        ...
        for line in lines:
            ...
            # Skip divider lines created by previous steps
            ...
            # 1. Keep if has digit
            # This handles cases like "(704) BRANCH TRAINING" (contains 704) and "(501)"
            if re.search(r"\d", line):
                keep = True

            # 2. Keep if it looks like a label (short text, e.g. < 60 chars)
            # This helps capture text that was split from its number by OCR
            # e.g. Line 1: "(501)", Line 2: "Computers" -> "Computers" kept here
            elif len(line) < 60:
                keep = True

            if keep:
                # Normalize for duplicate checking (lower case, remove extra spaces)
    ...
    def _process_pdf(self, path, method, lang):
        ...
        try:
            ...
            extracted_parts.append(
                f"Pages with Visuals: {stats['pages_with_drawings']}"
            )
            ...
            extracted_parts.append(
                "[System Info: PyMuPDF not installed, skipping structural pass]"
            )
        except Exception as e:
            extracted_parts.append(
                f"[System Info: Structural analysis failed: {str(e)}]"
            )
                ...
                try:
                    ...
                    if img_files:
                        ...
                        try:
                            ...
                            print(
                                "DEBUG: Tesseract engine not found. Skipping image OCR."
                            )
                            ...
                            for img_file in img_files:
                                try:
                                    ...
                                    if img_text.strip():
                                        ...
                                        print(
                                            f"DEBUG: No text found in {img_file.name}"
                                        )
                                except Exception as e:
